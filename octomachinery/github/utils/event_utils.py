...
_HttpHeaderStubs = _t.Sequence[_t.Mapping[str, str]]
"""A sequence of single-pair mappings, as stored in an event VCR."""

_DecodedHttpHeaders = _t.Union[_HttpHeaderStubs, _t.Mapping[str, str]]
"""Whatever the headers document in a VCR decodes into, valid or not."""

_EventStub = _t.Mapping[str, object]
"""A GitHub event payload, as stored in an event VCR."""

_EventFileContents = _t.Tuple[_DecodedHttpHeaders, _EventStub]
"""Whatever a single event VCR file decodes into."""


def _probe_yaml(event_file_fd: _t.IO[str]) -> _EventFileContents:
    try:
...
def _probe_jsonl(event_file_fd: _t.IO[str]) -> _EventFileContents:
    event = None
...
def _probe_json(event_file_fd: _t.IO[str]) -> _EventFileContents:
...
def _parse_fd_content(event_file_fd: _t.IO[str]) -> _EventFileContents:
...
def _transform_http_headers_list_to_multidict(
        headers: _DecodedHttpHeaders,
) -> multidict.CIMultiDict[str]:
    if isinstance(headers, _c.Mapping):
        raise ValueError(
...
def parse_event_stub_from_fd(
        event_file_fd: _t.IO[str],
) -> _t.Tuple[multidict.CIMultiDict[str], _EventStub]:
...
# NOTE: The values are `typing.Any` rather than `object` because the
# NOTE: body below consumes two of them as strings without narrowing.
# NOTE: Adding `isinstance()` guards would change which exception
# NOTE: escapes, so it is left to a follow-up change.
def validate_http_headers(headers: _t.Mapping[str, _t.Any]) -> None:
